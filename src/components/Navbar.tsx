  ...
  return (
    <motion.nav
      ...
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        isScrolled ? 'bg-blue-900 backdrop-blur-sm rounded-b-[2rem]' : 'bg-blue-900/20 backdrop-blur-sm'
      }`}
    >
      <div className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-10">
        {/* Top Row - Header */}
        <div className={`flex justify-between items-center h-16 transition-all duration-300 ${
          isScrolled ? 'border-b border-blue-700/50' : 'border-b border-white/30'
        }`}>
          {/* Logo */}
          <motion.div
          ...
          <div className="flex items-center space-x-6">
            <div className="flex items-center space-x-2">
              <span className="text-white/90 text-sm font-medium">USD</span>
              <div className="w-7 h-5 bg-white/90 rounded-md flex items-center justify-center shadow-sm">
                <span className="text-blue-900 text-xs font-bold">US</span>
              </div>
            </div>
            ...
            <button className="text-blue-900 bg-white hover:bg-gray-50 px-4 py-2 rounded-lg text-sm font-semibold transition-all duration-200 shadow-sm hover:shadow-md">
              Register
            </button>
          ...
          <div className="flex space-x-2">
            {navItems.map((item, index) => (
              <motion.a
                ...
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                className={`flex items-center px-6 py-3 rounded-full font-medium text-white transition-all duration-300 ${
                  index === 0 
        ...
        <AnimatePresence>
          {isOpen && (
            <motion.div
              ...
              className="md:hidden bg-blue-800 backdrop-blur-sm border-t border-blue-700"
            >
